...
class NepaliDateConverter:
    ...
    def bs2ad(self,bs_date):
        ...
        bs_dpate : A tuple in the format (year,month,day)
...
class PyApplet(object):
    def __init__(self):
        ...
        image.connect("activate",self.quit_ind)
        image.show()
        menu.append(image)
        menu.show()
        ind.set_menu(menu)
        ...
        # GTK setup
        gtk.timeout_add(1000, self.update_date, ind)
        gtk.main()
        sys.exit()

    def update_date(self, ind):
        date_str = contents_func()
        ind.set_label(date_str)
        return True

    def quit_ind(self, widget, data=None):
        gtk.main_quit()
        return 0;
